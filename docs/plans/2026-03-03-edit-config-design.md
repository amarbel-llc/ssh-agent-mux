...
1. Canonicalize the path (resolve symlinks) to find the real target file.
1. Copy to a temp file in the canonical target's parent directory (same
   filesystem guarantees atomic rename).
1. Open `VISUAL` or `EDITOR` on the temp file.
1. Compare temp file to original. If identical, print "No changes made" and
   exit.
1. Validate via `Config::validate_file()`. On failure, print the error and the
   temp file path, then exit non-zero.
1. `std::fs::rename()` the temp file over the canonical target (atomic).
1. Restart the service (reuse existing restart logic). If the service is not
